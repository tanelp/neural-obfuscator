    ...
    def obfuscate(self, img):
        ...
        for face in faces:
            ...
            if self.method == "pixelate":
                ...
                dlatents_encoded = self.encoder.encode(aligned_face, optim_image_size=256, num_steps=300)
                ...
                obfuscated_face_canvas = self.aligner.append_face_to_black_canvas(obfuscated_face, img, params["center"], method="eyes_nose", face_size=aligned_face.shape[0])
                face_mask = self.aligner.get_mask(landmarks, img.shape[0], img.shape[1])
